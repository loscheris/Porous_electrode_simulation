...
//	recordtime.resize(0);
	...
	int No_Timesteps = (int) (2*fabs(theta_v-theta_i)/deltaTheta+0.5);
	int No_potentialsteps;
	No_potentialsteps = (int) (2*fabs(theta_v-theta_i)/step_potential+0.5);
	int No_timesteps_per_potentialstep;
	No_timesteps_per_potentialstep = No_Timesteps/No_potentialsteps;
	...
	cout<<"step potential: "<<step_potential<<endl;
	cout<<"No. of Timesteps: "<<No_Timesteps<<endl;
	cout<<"No. of potential steps: "<<No_potentialsteps<<endl;
	cout<<"No. of timesteps per potential step: "<<No_timesteps_per_potentialstep<<endl;
	cout<<"Db: "<<db<<endl;
	...
	int change_timestep= No_timesteps_per_potentialstep/2;
	...
	for (int k = 0; k <No_Timesteps; ++k) {
		...
		if (k <= No_Timesteps/2 && k==change_timestep) {
			Theta -= step_potential;
			change_timestep += No_timesteps_per_potentialstep;
		} else if(k==change_timestep) {
			Theta += step_potential;
			change_timestep += No_timesteps_per_potentialstep;
		}
...
//		}
//		Jandtheta1<<k*deltaT<<"\t"<<flux1<<"\t"<<Theta<<"\n";
...
#pragma omp parallel for
		...
		Jandtheta2<<k*deltaT<<"\t"<<flux2<<"\t"<<Theta<<"\n";
