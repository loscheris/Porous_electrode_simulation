...
Ip = -0.446 * F * c* pi* re**2 * np.sqrt((db*F*D_A*v)/(R*T))
Jp = -0.446/2*np.sqrt(sigma)
...
plt.text(0.0, rescale*min(I)+0.6, 'Expected Ip = %.3e' %(Jp))
plt.text(-0.1, rescale*max(I),b'Peak to peak difference: %.3fV\nPeak I(dimensionless): %.3e' %(delta_E,min(I)))
