...
major_x_tick = np.arange(0.9, 1.011, 0.01)
minor_x_tick = np.arange(0.995, 1.005, 0.001)
major_y_tick = np.arange(0, 151, 30)
...
for frame in range(1,Ca.shape[0] / Z.size+1):
	...
	#--------------------------------------------------------------
	plt.gca().yaxis.set_major_formatter(mtick.FormatStrFormatter('%s'))
	plt.gca().set_yticks(major_y_tick)
	#plt.gca().set_yticks(minor_y_tick, minor=True)
	plt.gca().tick_params(axis='y', which = 'major', length=5, color='k', width=1, pad=10)	
	...
	plt.gca().tick_params(axis='x', which = 'major', length=5, color='k', width=1, pad=10)	
	...
	#x-ticks(label format, major+minor ticks)
	#-------------------------------------------------------------------
	plt.gca().xaxis.set_major_formatter(mtick.FormatStrFormatter('%0.2f'))
	plt.gca().set_xticks(major_x_tick)
	#plt.gca().set_xticks(minor_x_tick, minor=True)
	plt.gca().tick_params(axis='x', which = 'major', length=5, color='k', width=1, pad=10)	
	#plt.gca().tick_params(axis='x', which = 'minor', length=5, color='k')
